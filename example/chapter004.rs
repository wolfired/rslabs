...
fn section001() {
    {
        ...
        println!("{0}", _v0);
        // println!("{0}", _v1); // error code: 超出了_s1的作用域
    }

    {
        ...
        let _v0 = String::from("v0");
        let _v1 = _v0; // _v0被移动, 不再有效
...
fn section002() {
    let _v = P { age: 0 };
    let _vr = &_v; // _vr是对_v的引用
    unmove_ownership(&_v); // 借用_v, 本质上就是获得一个对_v的引用
    unmove_ownership(_vr);
    println!("{:?}, {:?}", _v, _vr);
}
